...
- ソースコード（実装・テスト含む）にマルチバイト文字を新規に書かない
- コードコメント・docstring・テストの説明文（`describe` / `context` / `it`）・spec 内のリテラル等はすべて ASCII（英語）で書く
- ユーザー向け表示文字列など、プロダクトの仕様としてマルチバイト文字が必要な箇所は `api/constants.rb` 等の定数経由で参照し、spec から比較する場合も定数を介する（spec ファイル本体にマルチバイトリテラルを書かない）
- 既存ファイル内の日本語コメントは、当該行を編集する場合を除き原則そのまま残す（差分を最小化するため）
