...
- エンジニアだけが読むコード上の表現にはマルチバイト文字を新規に書かない
  - コードコメント・docstring・テストの説明文（`describe` / `context` / `it`）等は ASCII（英語）で書く
- プロダクトの仕様としてユーザーに見せる文字列（UI 表示ラベル等）は日本語で OK
  - 例: `api/constants.rb` のラベル、エラーメッセージのうちユーザー向け表示など
